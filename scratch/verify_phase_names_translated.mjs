/**
 * Vietnamese phase names must actually be translated.
 *
 * The bug this guards: every `phases.<sword>.<n>.shortName` in src/i18n/vi.json was
 * copied verbatim from en.json, and the name portion of `phases.<sword>.<n>.name`
 * was left in English too ("Giai đoạn 1: Hunger"). The `effects` text WAS translated,
 * which is what made it look done. The HUD renders the phase name from
 * getPhaseInfo(), so a Vietnamese player saw "GIAI ĐOẠN: SPROUT".
 *
 * The check is "vi must differ from en", which is a real defect detector for these
 * fields but would false-positive on values that are legitimately the same in both
 * languages (proper nouns and sword names). Those go in ALLOW_IDENTICAL, so each
 * exception is a recorded decision rather than silence.
 */
