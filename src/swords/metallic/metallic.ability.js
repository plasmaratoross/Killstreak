/**
 * metallic — primary (Z), metallic, phase 4+.
 *
 * Extracted verbatim from js/game.js (Game.activateIronWill) in Phase 2. `this` was
...
export default {
  ...
  activate(game) {
    ...
    if (!game.player || !game.player.isSwordEquipped || game.player.swordId !== "metallic" || game.player.phase.phase < 4) return false;
    if (game.ironWillCooldown > 0) return false;

    game.ironWillCooldown = 17.0;
    game.player.ironWillActive = true;
    game.player.ironWillTimer = 6.9;
    ...
    game.floatingTexts.push(
      new FloatingText(game.player.x, game.player.y - 36, I18n ? I18n.t("floating.iron_will", { defaultValue: "IRON WILL!" }) : "IRON WILL!", "#f8fafc", 19)
    );

    for (let i = 0; i < 26; i++) {
      const pAngle = Math.random() * Math.PI * 2;
      const speed = 80 + Math.random() * 160;
      const pColor = i % 2 === 0 ? "#ffffff" : "#cbd5e1";
      game.particles.push(
