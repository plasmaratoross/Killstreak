/**
 * SaveManager — ES Module wrapper for persistent game data.
 * Extracted verbatim from js/storage.js — no logic changed.
 * Replaces window.Killstreak.Storage with a proper named export.
 */
...
const VALID_SWORDS = ["devourer", "overdrive", "aquatic", "soil", "metallic", "flora", "hellfire"];
...
const SaveManager = {
  load() {
    try {
      ...
      const currentLang = (parsed.settings && (parsed.settings.language === "vi" || parsed.settings.language === "en"))
        ...
        : "en";
      const eqSword = VALID_SWORDS.includes(parsed.equippedSword) ? parsed.equippedSword : "devourer";
      return {
        kills: unifiedKills,
      ...
      console.warn("[Killstreak SaveManager] Failed to load data, using defaults.", err);
  ...
  save(data) {
    ...
    const lang = data.settings && (data.settings.language === "vi" || data.settings.language === "en")
      ...
      : "en";
    const eqSword = VALID_SWORDS.includes(data.equippedSword) ? data.equippedSword : "devourer";
    ...
    try {
      ...
      console.warn("[Killstreak SaveManager] Failed to save data.", err);
    }
  },
  ...
  reset(keepLanguage = true) {
    ...
    try {
      ...
      console.warn("[Killstreak SaveManager] Failed to reset data.", err);
    }
    return freshSave;
  }
};
